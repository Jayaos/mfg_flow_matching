...
class CelebA64Benchmark(PotentialMapBenchmark):
    def __init__(
        ...
        resnet = load_resnet_G(str(CURRENT_DIR) + '/benchmarks/CelebA64/Final_G.pt')
        ...
        D1.load_state_dict(
            torch.load(
                str(CURRENT_DIR) + f'/benchmarks/CelebA64/{which}_v1.pt',
        ...
        D2.load_state_dict(
            torch.load(
                str(CURRENT_DIR) + f'/benchmarks/CelebA64/{which}_v2.pt',
                map_location=lambda storage, loc: storage
            )
        )
        freeze(D2)
...
class Mix3ToMix10Benchmark(PotentialMapBenchmark):
    def __init__(self, dim=2, batch_size=1024, device='cuda'):
        ...
        D1.load_state_dict(
            torch.load(
                str(CURRENT_DIR) + f'/benchmarks/Mix3toMix10/{dim}_v1.pt',
                map_location=lambda storage, loc: storage
            )
        )
        freeze(D1)
        ...
        D2.load_state_dict(
            torch.load(
                str(CURRENT_DIR) + f'/benchmarks/Mix3toMix10/{dim}_v2.pt',
                map_location=lambda storage, loc: storage
            )
        ) 
        freeze(D2)
