...
class LinearFlowMatcher:
    """
    Base class for linear flow matching method.
    """
    ...
    def sample_location_and_conditional_flow(self, x0, x1):
        """
        Compute the linear interpolant of x0 and x1 t*x1 + (1-t)*x0
        """
        ut = x1 -x0
        t = torch.rand(ut.shape[0])
        if x0.dim() == 4:
            # image
            t_expanded = t.view(-1, 1, 1, 1).expand(-1, *ut.shape[1:])
        if x0.dim() == 2:
            t_expanded = t.view(-1, 1).expand(-1, *ut.shape[1:])
        pt = x0 + ut * t_expanded
